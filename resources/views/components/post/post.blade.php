<div class="border d-flex flex-col p-3 mb-4">
    ...
    <div class="w-100">
        ...
        <p>{{ $post->message }}</p>
        <div class="w-25">
            <form action="{{ route('timeline.like', $post->id) }}" method="POST">
                @csrf
        ...
        <div class="d-flex align-items-center justify-content-between w-100"> 
            ...
            <div class="d-flex align-items-center justify-content-end">
                @if ($post->user_id == Auth::user()->id)
                    <form action="{{ route('timeline.like', $post->id) }}" method="POST" class="border" style="width: 10%; height: 10%">
                        @csrf
                        @method('DELETE')
                        <button class="btn">
                ...
                @endif
                <form action="{{ route('timeline.like', $post->id) }}" method="POST" class="border" style="width: 10%; height: 10%; margin-left: 15px">
                    @csrf
                    @method('PUT')
                    <button class="btn">
                        <svg xmlns="http://www.w3.org/2000/svg" style="width: 100%" fill="none" viewBox="0 0 24 24" stroke="orange">
